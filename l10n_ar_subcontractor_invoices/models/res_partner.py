...
class ResPartner(models.Model):
    ...
    subcontractor_pos_number = fields.Integer(
        string="Subcontractor POS",
        help="Point of sale used to post subcontractor invoices",
    )
    subcontractor_invoice_ids = fields.One2many(
        "account.move", "subcontractor_partner_id", "Subcontractor invoices"
    )
    subcontractor_invoice_count = fields.Integer(
        "Subcontractor invoice count",
        compute="_compute_subcontractor_invoice_count",
        store=True,
    )
    ...
    def action_subcontractor_fetch_pos(self):
        ...
        for record in self:
            ...
            if not res:
                ...
                _logger.warning(
                    "Subcontractor %s has %s POS: %s", record.name, len(res), res
                )
            else:
                pos = res[0].split("|")[0]
    ...
    def action_open_subcontractor_invoices(self):
        self.ensure_one()
        action = self.env.ref(
            "l10n_ar_subcontractor_invoices.action_subcontractor_invoice_type"
        ).read()[0]
        action["context"] = {}
