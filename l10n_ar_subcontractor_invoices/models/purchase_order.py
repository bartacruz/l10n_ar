...
class PurchaseOrder(models.Model):
    ...
    subcontractor_customer_id = fields.Many2one(
        "res.partner", string="Purchase customer"
    )
    ...
    def _compute_invoiced_by_subcontractor(self):
        for record in self:
            ibs = [
                line.product_id.invoiced_by_subcontractor for line in record.order_line
            ]
            if not all(ibs) and any(ibs):
                ...
                _logger.warning(
                    _(
                        "Subcontractor records must have ALL subcontracted lines. "
                        "problematic record: %s" % record
                    )
                )
            record.invoiced_by_subcontractor = all(ibs) and any(ibs)
            if (
                record.invoiced_by_subcontractor
                and record._get_sale_orders().partner_id
            ):
                record.subcontractor_customer_id = (
                    record._get_sale_orders().partner_id.id
                )
        ...
        if self.invoiced_by_subcontractor and self.subcontractor_customer_id:
            ...
            fiscal_position_id = (
                ...
                or self.fiscal_position_id.get_fiscal_position(
                    self.subcontractor_customer_id.id
                )
            )
            ...
            vals["journal_id"] = (
                self.env["account.move"]._get_subcontractor_journal(["sale"]).id
            )
        return vals
