...
class AccountMove(models.Model):
    ...
    # TODO: agregar un constraint. Si es invoiced_by_subcontractor TIENE que tener el partner seteado
    invoiced_by_subcontractor = fields.Boolean(
        compute="_compute_invoiced_by_subcontractor",
        string="Invoiced by subcontractor",
        store=True,
    )
    subcontractor_partner_id = fields.Many2one(
        "res.partner", string="Invoice subcontractor"
    )
    invoiced_by_display_name = fields.Char(
        "Invoiced by", compute="_compute_invoiced_by", store=True
    )
    ...
    def _get_subcontractor_journal(self, journal_types):
        ...
        domain = [
            ...
            ("invoiced_by_subcontractor", "=", True),
        ]
    ...
    def _compute_invoiced_by_subcontractor(self):
        for record in self:
            ibs = [
                line.product_id.invoiced_by_subcontractor
                for line in record.invoice_line_ids
            ]
            if not all(ibs) and any(ibs):
                raise ValidationError(
                    _(
                        "Mixing subcontracted product lines with normal ones"
    ...
    def _compute_l10n_latam_document_type(self):
        ...
        ret = super(AccountMove, no_subinvoiced)._compute_l10n_latam_document_type()
        for record in self - no_subinvoiced:
            ...
            subcontractor_letters = issue_letters[
                record.subcontractor_partner_id.l10n_ar_afip_responsibility_type_id.code
            ]
            customer_letters = receive_letters[
                record.partner_id.l10n_ar_afip_responsibility_type_id.code
            ]
            letters = list(set(subcontractor_letters) & set(customer_letters))
            types = record.env["l10n_latam.document.type"].search(
                [
                    "&",
    ...
    def _check_unique_sequence_number(self):
        no_subinvoiced = self.filtered(lambda x: not x.invoiced_by_subcontractor)
        return super(AccountMove, no_subinvoiced)._check_unique_sequence_number()
    ...
    def _get_last_sequence_domain(self, relaxed=False):
        ...
        if self.subcontractor_partner_id:
            ...
            # proveedor
            where_string += (
                " AND  subcontractor_partner_id= %(subcontractor_partner_id)s"
            )
    ...
    def _check_argentinean_invoice_taxes(self):
        ...
        return super(AccountMove, moves)._check_argentinean_invoice_taxes()

    def create(self, vals):
        if "subcontractor_partner_id" in vals:
            partner = self.env["res.partner"].browse(
                vals.get("subcontractor_partner_id")
            )
            ctx = {
    ...
    def _post(self, soft=True):
        ...
        for inv in subcontractor_invoices:
            ...
            # post with context to AFIP WS
            res = res + super(AccountMove, inv.with_context(**ctx))._post(soft)
        return res
